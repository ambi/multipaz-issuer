...
│   └── src/main/kotlin/org/multipaz/issuer/
│       ├── Application.kt
...
│           └── routes/{Auth, Home, Oid4vci}Routes.kt
└── verifier/                              # Verifier アプリケーション
    └── src/main/kotlin/org/multipaz/verifier/
        ├── Application.kt
        ├── application/VerifyCredentialUseCase.kt
        ├── domain/verification/           # 検証ドメイン
        ├── infrastructure/multipaz/MdocVerifier.kt
        └── web/
            ├── plugins/{Routing, Serialization}.kt
            └── routes/VerifierRoutes.kt
```
...
# テスト
./gradlew :issuer:test
./gradlew :issuer:test --tests "org.multipaz.issuer.domain.credential.CredentialIssuanceServiceTest"
...
Issuer と Verifier は DDD 的に別ドメインであるため、独立したアプリケーションとして分離している。共有コードはなく、パッケージ名も `org.multipaz.issuer` と `org.multipaz.verifier` で独立している。
...
`InMemoryIssuanceSessionRepository` / `InMemoryVerificationSessionRepository` は `ConcurrentHashMap` で実装されており、**サーバー再起動でリセットされる**。本番環境では Redis や DB 等の永続化ストアに差し替える。
...
c_nonce は `NonceStore` で独立して管理する。OID4VCI の `/nonce` エンドポイントは認証なしで呼ばれるため、`IssuanceSession` とは分離している。nonce は one-time use で、TTL は 600 秒。
