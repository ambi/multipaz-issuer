...
class CredentialIssuanceServiceTest {
    private fun createService(): CredentialIssuanceService = CredentialIssuanceService(InMemoryIssuanceSessionRepository())
    ...
    @Test
    fun `exchangePreAuthorizedCode は有効なコードで TOKEN_ISSUED に遷移する`() =
        runTest {
            val service = createService()
            val session = service.createSession(sampleCredential())
    ...
    @Test
    fun `exchangePreAuthorizedCode は無効なコードで null を返す`() =
        runTest {
            val service = createService()
    ...
    @Test
    fun `同じ pre-authorized_code を 2 回使うと 2 回目は null を返す`() =
        runTest {
            val service = createService()
            val session = service.createSession(sampleCredential())
    ...
    @Test
    fun `findSessionByAccessToken は TOKEN_ISSUED セッションを返す`() =
        runTest {
            val service = createService()
            val session = service.createSession(sampleCredential())
    ...
    @Test
    fun `markCredentialIssued は状態を CREDENTIAL_ISSUED に更新する`() =
        runTest {
            val service = createService()
            val session = service.createSession(sampleCredential())
