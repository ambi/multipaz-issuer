...
| リバースプロキシ対応 | Ktor ForwardedHeaders プラグイン（`ktor-server-forwarded-header`） |
...
TRUSTED_PROXY_COUNT=1                         # 前段のリバースプロキシ数（デフォルト: 1）
                                              # Fly.io / nginx 1 台なら 1、直接公開なら 0
NONCE_HMAC_SECRET=<64 hex chars>             # c_nonce 署名鍵（openssl rand -hex 32 で生成）
                                              # 未設定時は起動ごとにランダム生成（単一インスタンスのみ有効）
...
#### issuer-keystore.p12 のバックアップと復元

**キーストアを紛失すると、既に発行済みの Photo ID が Wallet で無効化（署名検証失敗）になる。** 定期的にバックアップすること。

```bash
# バックアップ（Fly.io volumes の場合）
fly ssh console -c fly.issuer.toml
cp /app/data/issuer-keystore.p12 /app/data/issuer-keystore-$(date +%Y%m%d).p12

# ローカルにコピー（fly sftp または scp 相当）
fly ssh sftp get /app/data/issuer-keystore.p12 ./issuer-keystore-backup.p12 -c fly.issuer.toml

# 復元（バックアップからのリストア）
fly ssh sftp shell -c fly.issuer.toml
# put local-path /app/data/issuer-keystore.p12
```

キーストアの内容確認（パスワードを用いて証明書の有効期限等を確認できる）：

```bash
keytool -list -v -keystore issuer-keystore.p12 -storetype PKCS12 -storepass <KEY_STORE_PASSWORD>
```
...
`/nonce`・`/token`・`/credential` エンドポイントにクライアント IP ベースのレート制限を適用している。

| エンドポイント | 上限（60 秒ウィンドウ） |
| -------------- | ---------------------- |
| `POST /nonce` | 60 req/IP |
| `POST /token` | 10 req/IP |
| `POST /credential` | 5 req/IP |

実装は `RateLimiterPort` インターフェースで抽象化されており、`REDIS_URL` の有無で自動切り替えする。

| モード | 実装 | 特性 |
| ------ | ---- | ---- |
| インメモリ | `RateLimiter`（スライディングウィンドウ） | 単一インスタンス向け |
| Redis | `RedisRateLimiter`（固定ウィンドウ: `INCR` + `PEXPIRE`） | 複数インスタンスで共有 |

Redis 版は固定ウィンドウ方式のためウィンドウ境界で最大 2× のバーストを許容するが、不正利用防止の用途には十分な精度である。Redis 障害時はフェイルオープン（リクエストを許可）して可用性を優先する。

### クライアント IP 解決（X-Forwarded-For）

Ktor の `XForwardedHeaders` プラグインを使い、`X-Forwarded-For` から正しいクライアント IP を `request.origin.remoteHost` に設定する。
